...
| Signal Explorer dataset | Built from the three feeds above, but from January 1, 2021 onward (growing until it holds 15 years, then the oldest year drops off) (FDA and FSIS recalls, FSIS public health alerts, CDC notices). Each record is classified by hazard type, organism or allergen, product category, geography, root cause, detection, corrective action, and status; fields the agency did not give are marked "not stated", and fields our rules assigned are marked "inferred". Every build is compared with the previous one and the differences go to `data/signals-changelog.json`. | Every 6 hours |
