// One HTTP client for every fetcher. CDC and USDA sit behind bot protection that
// rejects requests without browser-like headers, so every request goes through here.
...
export async function fetchText(url: string, opts: FetchOptions = {}): Promise<string> {
  const { timeoutMs = 30_000, retries = 2, accept = "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8" } = opts;
  let lastErr: unknown;
